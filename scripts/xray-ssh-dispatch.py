...
def validate_argv(argv: list[str]) -> None:
    ...
    if spec is None:
        fail(f"Команда не разрешена: {argv[0]}")
    ...
    while index < len(argv):
        ...
        if token not in flags:
            fail(f"Аргумент не разрешён для {command}: {token}")
        expects_value = flags[token]
        if expects_value:
            index += 1
            if index >= len(argv):
                fail(f"Отсутствует значение для {token}")
            if "\x00" in argv[index]:
                fail(f"Некорректное значение для {token}")
        index += 1
...
def main() -> None:
    ...
    if not original_command:
        fail("Интерактивный SSH для этой учётной записи отключён.", 127)

    try:
        argv = shlex.split(original_command)
    except ValueError as error:
        fail(f"Не удалось разобрать SSH_ORIGINAL_COMMAND: {error}")

    if not argv:
        fail("Пустой SSH_ORIGINAL_COMMAND.", 127)
